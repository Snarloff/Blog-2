<template>
  <section class="miniArticle">
    ...
    <div class="miniArticle__article" v-for="article in data" :key="article.id">
      <h4 class="miniArticle__subtitle">Em destaques / {{ category }} / {{ subcategory }}</h4>
      <div class="miniArticle__article-box">
        ...
        <router-link class="miniArticle__button" :to="{name: 'Article', query: { 
          category: category, 
  ...
  export default {
    data(){
      return {
        ...
        category: undefined,
        subcategoryId: String(this.$route.query.id),
        data: undefined
      }
    },
    ...
    watch: {
      '$route.query.subcategory': function(){
        this.subcategoryId = String(this.$route.query.id),
        this.executeData()
      }
    },
    ...
    methods: {
      ...
      async executeData(){
        ...
        await axios.get(String(url) + '/api/v1/articles?limit=3&subcategoryId=' + this.subcategoryId).then(async (data) => {
          ...
          await axios.get(String(url) + '/api/v1/subcategories?id=' + this.subcategoryId + '&suffix=true').then(async (subcat) => {
            if (!subcat['data']['status']) {
              this.error = subcat['data']['data']
            }
            this.subcategory = subcat['data']['data'][0]['title']
            let categoryId = subcat['data']['data'][0]['categoryId']

            await axios.get(String(url) + '/api/v1/categories?id=' + categoryId).then((cat) => {
              if (!cat['data']['status']){
                this.error = subcat['data']['data']
              }
              this.category = cat['data']['data']['title']
            })
